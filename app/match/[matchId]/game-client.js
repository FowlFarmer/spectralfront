...
const describeResult = result => result.unstable ? 'Unstable function — turn lost.' : result.hit ? 'Direct hit. One ship lost.' : result.impact?.kind === 'asteroid' ? 'Asteroid vaporized.' : result.impact?.kind === 'planet' ? 'Planetary impact. Arc stopped.' : 'Beam exits the sector.';

export default function GameClient({ matchId }) {
  const router = useRouter(), isBotMatch = matchId.startsWith('bot-');
  const peer = useRef(null), channel = useRef(null), session = useRef(null), gameRef = useRef(null), botTimer = useRef(null), botTurnKey = useRef(null), shotTimer = useRef(null), resolveTimer = useRef(null), shotAnimating = useRef(false);
  ...
  const restartBotMatch = useCallback(() => { clearTimeout(botTimer.current); botTurnKey.current = null; clearTimeout(shotTimer.current); clearTimeout(resolveTimer.current); shotAnimating.current = false; setSelected(0); setNotice('New training sector initialized.'); publish(createMatch()); }, [publish]);

  const commitAction = useCallback(action => {
    const result = applyGameAction(gameRef.current, action);
    if (result.ignored) return null;
    lockShot(); publish(result.game); setNotice(result.pending ? 'Arc in flight.' : describeResult(result));
  ...
  const completePendingShot = useCallback(shotId => {
    ...
    publish(result.game); setNotice(describeResult(result));
    if (!isBotMatch) send({ type: 'state', state: result.game });
  }, [isBotMatch, publish, send]);
  ...
  }, [commitAction, isBotMatch, lockShot, send]);
  ...
  return <div className="game-shell"><header className="game-top"><div className="brand">SPECTRAL <i>FRONT</i></div><div className="match-meta"><span className="online">● {link}</span> &nbsp; {isBotMatch ? 'TRAINING MATCH' : `MATCH ${matchId.slice(0, 6).toUpperCase()}`}</div><button className="leave" onClick={leave}>LEAVE MATCH</button></header><main className="game-grid"><aside className="panel side"><div><div className="label">TURN</div><div className="turn">{matchOver ? (won ? 'Sector secured' : 'Fleet lost') : game.pendingShot ? 'Arc in flight' : game.turn === me ? 'Your turn — plot a line' : isBotMatch ? 'Bot is plotting' : 'Rival is plotting'}</div></div><div><div className={'player ' + (game.turn === me && !game.pendingShot ? 'active' : '')}><strong>YOUR FLEET</strong><small>{matchOver ? `${liveShips(game, me).length} SURVIVING` : game.pendingShot ? 'HOLDING FIRE' : 'SELECT A SHIP'}</small><div className="ship-select">{myShips.map((ship, index) => <button key={index} disabled={!ship.hp || matchOver || !!game.pendingShot} className={'ship-choice ' + (index === selected ? 'selected' : '')} onClick={() => setSelected(index)}>SHIP {index + 1}<i>{ship.hp ? 'READY' : 'LOST'}</i></button>)}</div></div><div className={'player enemy ' + (game.turn === foe && !game.pendingShot ? 'active' : '')}><strong>{isBotMatch ? 'BOT FLEET' : 'RIVAL FLEET'}</strong><small>{matchOver ? `${liveShips(game, foe).length} SURVIVING` : game.pendingShot ? 'IMPACT PREDICTED' : isBotMatch ? 'NAVIGATION AI' : 'OPPOSING FLEET'}</small><div className="dots">{'● '.repeat(liveShips(game, foe).length) || '—'}</div></div></div><div className="rules">Pick a ship. It is <b>(0,0)</b> for this shot. The field remains Cartesian: left is −x, right is +x, up is +y.</div></aside><section className="panel arena-wrap"><div className="arena-top"><span>LOCAL SIMULATION: <b>{isBotMatch ? 'BOT TRAINING' : me === 'host' ? 'HOST' : 'CONNECTED'}</b></span><span>{matchOver ? 'MATCH COMPLETE' : game.pendingShot ? 'ARC IN FLIGHT' : `ROUND ${String(game.round).padStart(2, '0')}`}</span></div><ArenaCanvas game={game} role={me} selected={selected} onSelectShip={setSelected} /><div className={'event ' + (notice ? 'show' : '')}>{notice}</div>{matchOver && <MatchConclusion won={won} isBotMatch={isBotMatch} myRemaining={liveShips(game, me).length} foeRemaining={liveShips(game, foe).length} onRestart={restartBotMatch} onLeave={leave} />}<section className="command"><div className="formula"><label>FIRING ARC — Y = F(X)</label><div className="formula-row"><span>y =</span><input value={formula} disabled={matchOver || !!game.pendingShot} onChange={event => setFormula(event.target.value)} autoComplete="off" spellCheck="false" /></div><div className="hint">{matchOver ? 'Command channel closed — match result confirmed.' : game.pendingShot ? 'Trajectory locked until the beam resolves.' : 'Origin: selected ship (0, 0) · sin, cos, abs, sqrt, log, exp'}</div></div><button className="fire" disabled={game.turn !== me || matchOver || !!game.pendingShot} onClick={fireCurrent}>{matchOver ? 'MATCH ENDED' : game.pendingShot ? 'ARC IN FLIGHT' : 'FIRE ARC'}</button></section></section></main></div>;
}
